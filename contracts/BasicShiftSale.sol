...
contract ShiftSale {
    ...
    modifier validRaisedAmount(uint amount) {
        require(amount <= raised);
        _;
    }
    ...
    function purchase(uint amount)
    ownerExists(msg.sender)
    validRaisedAmount(amount)
    public
    {
    ...
    /// @param _to Destination address.
    /// @param _value quantity of MTC tokens to transfer.
    function transferMany(address[] _to, uint256[] _value)
    ownerExists(msg.sender)
    public {
